...
def update_length(state):
    # Direct update of the dictionary is not allowed:
    #state["metric.average_length"] += 1
    #state["metric"]["average_length"] += 1
    # Updating the dictionary and reinserting it triggers an event.
    # The status message is not changed by the following code!
    metrics = state["metrics"]
    metrics["average_length"] += 1
    state["metrics"] = metrics
